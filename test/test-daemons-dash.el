...
(ert-deftest daemons-dash-test-sprite-provider ()
  ...
  (cl-letf (((symbol-function 'require)
             (lambda (feat &rest _) (eq feat 'sprite)))
            ((symbol-function 'sprite-resolve-list)
             (lambda () '((:name "test-sprite"))))
            ((symbol-function 'sprite-name)
             (lambda (s) (plist-get s :name)))
    ...
    (let ((items (daemons-dash-sprite-list)))
      ...
      (should (eq (daemons-dash-item-status (car items)) 'active)))))
