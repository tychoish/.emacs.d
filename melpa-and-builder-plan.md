...
**3. Reduce questionable dependencies**
...
- `cl-lib "1.0"`: `cl-lib` is bundled with Emacs since 24.3; the version pin is
...
**1. Fix the `URL:` header** (`annotated-completing-read.el:8`)
...
**3. Add `xlib` to `Package-Requires`** (line 6)
...
### 1 — Fix the UX flow (confusing selections)
...
higher priority). Update `builder--get-candidates` to return a sorted list of
candidates rather than a raw hash table. Define priority tiers:
...
Pass a sorted list (not a hash table) to `annotated-completing-read`.
`annotated-completing-read` already uses `ht-keys` for the collection;
switching to an ordered sequence preserves insertion order in the completion UI
with vertico.

---

### 3 — Reduce noisy candidates
...
| `builder-rust.el` | Rust / Cargo candidates | `rust-mode-hook`, `rust-ts-mode-hook` |
